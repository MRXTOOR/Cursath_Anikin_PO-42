...
class SiteConfigView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            ...
            school_slogan = request.POST.get("school_slogan")
            school_address = request.POST.get("school_address")
            # Обновите модель SiteConfig
            site_config_slogan = SiteConfig.objects.get(key="school_slogan")
            site_config_slogan.value = school_slogan
            site_config_slogan.save()

            site_config_address = SiteConfig.objects.get(key="school_address")
            site_config_address.value = school_address
            site_config_address.save()

            # Передайте site_defaults в контекст для использования в шаблоне
            context = {
                "formset": formset,
                "title": "Configuration",
                "site_defaults": {
